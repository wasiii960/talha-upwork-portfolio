...
export const caseStudies: CaseStudy[] = [
  ...
  },
  {
    ...
    tagline:
      "A microservices-based digital wallet and payment platform built on Spring Cloud — OAuth2/JWT authentication, service discovery, and centralized configuration behind a backend that has sustained roughly 99.9% uptime.",
    technologies: ["Java", "Spring Boot", "Spring Cloud", "OAuth2 / JWT", "PostgreSQL", "Redis"],
    accent: "from-emerald-500/20 via-cyan-400/10 to-transparent",
    heroImage: "/case-studies/onezapp-enterprise-digital-wallet-platform.jpg",
    executiveSummary:
      "OneZapp is an enterprise digital wallet and payment platform that lets users securely authenticate, manage wallets, move money, pay bills, and transact across a distributed backend of independent financial microservices. The platform is built on Spring Cloud, with OAuth2-based authentication, centralized configuration, and automatic service discovery underpinning every request. As backend software engineer on the platform, I designed and built the Authentication Service — implementing OAuth2.0 login and JWT token issuance with embedded roles and permissions for stateless authorization — configured the Eureka-based Discovery Server and Spring Cloud Gateway that route and secure every request reaching the platform, and stood up the Config Server that centralizes configuration across all backend services. I also optimized the Accounts Service's collection-processing logic using the Java Stream API, cutting data manipulation overhead by roughly 30%, and contributed to a backend architecture that has sustained approximately 99.9% service uptime through centralized configuration, resilient service discovery, and loosely coupled service communication.",
    businessProblem:
      "The organization needed a fintech backend capable of supporting wallets, transfers, bill payments, and top-ups as independent services, without those services becoming tightly coupled to each other or to a single point of failure. Every new capability — a new payment rail, a new reporting requirement, a new partner integration — needed to be addable without destabilizing the rest of the platform. That meant the backend had to solve, from day one: secure, stateless authentication that could scale across many independent services; a way to route external traffic to the right internal service without hardcoding network locations; a single source of truth for configuration instead of duplicated properties files scattered across services; and a deployment model resilient enough to sustain high availability as more services were added. Getting the foundational architecture right — authentication, gateway, discovery, and configuration — mattered more to the platform's long-term trajectory than any single feature, because every future service would inherit whatever patterns were established here.",
    challenges: [
      "Authenticating users once and propagating that identity securely across many independent services, without forcing every service to independently validate credentials against a shared session store, which would have reintroduced the very coupling the microservices architecture was meant to avoid.",
      "Routing all inbound traffic through a single, secure entry point while still allowing each backend service to evolve, scale, and deploy independently, without clients needing to know the internal network topology of the platform.",
      "Eliminating hardcoded service locations so that services could scale horizontally, restart, or be redeployed without breaking every other service that depended on them.",
      "Reducing duplicated configuration scattered across services — database URLs, feature flags, and environment-specific settings had been copy-pasted service to service, creating drift and making environment promotion risky.",
      "Validating JWT tokens on every request efficiently enough that authentication didn't become a latency bottleneck at the gateway, the single point every request passed through.",
      "Improving the Accounts Service's collection-processing logic, which was a measurable, day-to-day performance cost as account and transaction volumes grew, without a data model redesign.",
      "Achieving high availability across a growing number of independent services, where any single service's instability could not be allowed to cascade into an outage for the whole platform.",
    ],
    solution:
      "I approached the problem as a set of foundational platform concerns to get right once, rather than solving them independently inside every service. Authentication was centralized into a dedicated Authentication Service implementing OAuth2.0: it validates user credentials, issues signed JWT access tokens, and embeds user roles and permissions directly inside the token payload, so every downstream service can authorize a request statelessly, without a database lookup or a shared session store. All inbound traffic — from the mobile app and the web portal — passes through a Spring Cloud Gateway, which validates the JWT on every request before routing it to the correct backend service. Instead of hardcoding where each service lives, every service registers itself with a Eureka Discovery Server on startup, and the gateway resolves service locations dynamically. Configuration for every service is centralized in a Spring Cloud Config Server, so environment-specific settings live in one place instead of being duplicated service to service. On top of this foundation, I optimized the Accounts Service's collection processing using the Java Stream API, and the resulting architecture has sustained close to 99.9% uptime in production.",
    diagram: "gateway",
    diagramLayers: [
      { label: "Client Applications", nodes: ["Mobile Application", "Web Portal"] },
      { label: "Gateway & Security", nodes: ["Spring Cloud Gateway", "OAuth2 / JWT Validation"] },
      { label: "Discovery & Core Services", nodes: ["Eureka Discovery Server", "Account Service", "Wallet Service", "Payment Service", "Transfer Service", "Notification Service"] },
      { label: "Configuration & Data", nodes: ["Config Server", "PostgreSQL / Oracle", "Redis Cache"] },
    ],
    architectureNotes: [
      "Authentication flow: a client submits credentials to the Authentication Service, which validates them against Spring Security, generates a signed JWT access token embedding the user's identity, roles, and permissions, and returns it to the client — after which every subsequent request simply presents that token rather than re-authenticating.",
      "JWT structure: each token carries a header identifying the signing algorithm, a payload embedding user identity, roles, permissions, and expiry, and a signature that lets any service verify the token hasn't been tampered with, all without a network call back to the Authentication Service.",
      "API Gateway flow: the Spring Cloud Gateway intercepts every inbound request, validates the JWT's signature and expiry, applies rate limiting, resolves the target service via the Discovery Server, and routes the request onward — rejecting anything that fails validation before it reaches a backend service at all.",
      "Service discovery: every service registers itself with the Eureka Discovery Server on startup and sends regular heartbeats; the gateway and other services resolve target instances dynamically, so scaling a service up or down, or redeploying it, never requires reconfiguring the services that depend on it.",
      "Configuration management: the Spring Cloud Config Server centralizes environment-specific configuration — database connection details, feature flags, service-level settings — in one version-controlled source, replacing configuration that had previously been duplicated file-by-file across services.",
      "Security design: Spring Security enforces OAuth2/JWT validation at the gateway and, defense-in-depth, again inside sensitive services; all inter-service communication and client-facing APIs are encrypted, and authorization decisions are made from roles and permissions embedded in the token rather than a mutable, centrally-queried permission store.",
      "REST API design: each service exposes a narrow, resource-oriented REST API (accounts, wallets, transfers, payments, notifications) versioned independently, so a breaking change in one service's API doesn't force a synchronized release across the platform.",
      "Performance optimization: the Accounts Service's collection processing — filtering, grouping, and aggregating account and transaction data — was rewritten using the Java Stream API in place of imperative loops, reducing data manipulation overhead by approximately 30%.",
      "Testing strategy: JUnit and Mockito cover authentication logic, JWT generation and validation, and service-level business logic, with particular attention to token expiry, malformed-token, and unauthorized-access paths that are easy to under-test but critical in a financial platform.",
      "Deployment and monitoring: services are containerized with Docker behind a load balancer, deployed as independently scalable instances, with the Discovery Server and Config Server acting as the platform's operational backbone — keeping deployment and monitoring straightforward even as the number of services grows.",
    ],
    techStack: [
      { category: "Languages", items: ["Java"] },
      { category: "Frameworks", items: ["Spring Boot", "Spring Cloud", "Spring Security"] },
      { category: "Security", items: ["OAuth2", "JWT"] },
      { category: "Data", items: ["PostgreSQL", "Oracle", "Redis"] },
      { category: "Infrastructure", items: ["Spring Cloud Gateway", "Eureka Discovery Server", "Spring Cloud Config", "Docker", "Git"] },
      { category: "Quality", items: ["JUnit", "Mockito"] },
    ],
    role: [
      "Designed and developed the Authentication Service using Spring Security, implementing OAuth2.0 login and validating user credentials before issuing access tokens.",
      "Implemented JWT access token generation, embedding user roles and permissions directly inside the token to enable stateless authorization across every downstream service.",
      "Designed and configured the Spring Cloud Discovery Server (Eureka) so every backend service registers and is discovered automatically, without hardcoded service locations.",
      "Configured the Spring Cloud Gateway to route incoming requests to the correct backend microservice while validating OAuth2 JWT tokens on every request.",
      "Implemented the Spring Cloud Config Server for centralized configuration management, replacing duplicated configuration files across services with a single source of truth.",
      "Improved the Accounts Service's performance by rewriting collection-processing logic with the Java Stream API, reducing data manipulation overhead by approximately 30%.",
      "Contributed to a backend architecture that has sustained approximately 99.9% service uptime through centralized configuration, resilient service discovery, and loosely coupled service communication.",
      "Participated in backend API development, debugging, production support, and ongoing performance optimization across the platform.",
    ],
    engineeringDecisions: [
      {
        title: "OAuth2 + JWT over server-side sessions",
        detail:
          "Stateless JWT-based authentication was chosen over server-side session storage specifically because the platform was going to keep adding independent services. A shared session store would have become both a coupling point and a scaling bottleneck; embedding roles and permissions in a signed token let every service authorize requests independently, at the cost of tokens being harder to revoke instantly — a trade-off accepted given short token expiry windows.",
      },
      {
        title: "API Gateway as the platform's single entry point",
        detail:
          "Rather than exposing each microservice directly to clients, every request passes through a single Spring Cloud Gateway. This centralized JWT validation, rate limiting, and routing in one place instead of duplicating that logic into every service, at the cost of the gateway becoming a critical path component that had to be built for resilience and low latency.",
      },
      {
        title: "Service discovery over hardcoded service locations",
        detail:
          "Introducing a Eureka Discovery Server meant every service could scale, restart, or move without any other service needing reconfiguration. The trade-off was operational: the discovery server itself became a piece of infrastructure that needed to be highly available, since a discovery outage would affect the whole platform's ability to route requests.",
      },
      {
        title: "Centralized configuration over per-service config files",
        detail:
          "Moving configuration into a Spring Cloud Config Server eliminated a recurring source of environment drift, where a setting updated in one service's properties file was easy to forget to replicate elsewhere. This centralized a dependency — every service now needs the Config Server available at startup — mitigated by having services cache their last-known configuration.",
      },
      {
        title: "Java Streams over imperative loops in the Accounts Service",
        detail:
          "The Accounts Service's collection processing was rewritten using the Java Stream API instead of nested imperative loops. Beyond the roughly 30% reduction in data manipulation overhead, the resulting code was noticeably easier to reason about and test, though it required the team to build shared familiarity with functional-style collection pipelines.",
      },
      {
        title: "Independently versioned REST APIs per service",
        detail:
          "Each service's REST API is versioned on its own schedule rather than as part of a platform-wide release. This let services such as the Wallet Service and Payment Service evolve independently, at the cost of needing clear API contracts and gateway-level routing rules per version.",
      },
    ],
    implementation: [
      "Implemented the OAuth2 login flow end-to-end: credential validation against Spring Security, JWT generation with embedded roles and permissions, and token issuance back to the client.",
      "Built JWT validation as gateway middleware, checking signature, expiry, and embedded claims before a request is ever routed to a backend service.",
      "Registered every backend service (Account, Wallet, Payment, Transfer, Notification, Reporting) with the Eureka Discovery Server, with health checks and heartbeats keeping the service registry accurate.",
      "Configured Spring Cloud Gateway routing rules mapping inbound paths to the correct backend service via the Discovery Server, including request filtering and rate limiting at the gateway layer.",
      "Stood up the Spring Cloud Config Server with environment-specific profiles, migrating configuration out of individual services' properties files into a single, version-controlled source.",
      "Rewrote the Accounts Service's collection processing logic using the Java Stream API, replacing nested loops with declarative filter/map/reduce pipelines.",
      "Wrote JUnit and Mockito test suites covering authentication, JWT generation and validation, and gateway routing logic, with dedicated coverage for expired-token and malformed-token scenarios.",
      "Containerized services with Docker for consistent builds across environments, deployed behind a load balancer with multiple gateway instances for resilience.",
      "Provided production support and debugging across the platform, working from monitoring dashboards to trace authentication and routing issues back to their root cause.",
    ],
    performance: [
      { metric: "Accounts Service data manipulation overhead", before: "Baseline (imperative loops)", after: "~30% reduction (Java Stream API)" },
      { metric: "Service uptime", before: "n/a (pre-microservices)", after: "~99.9%" },
      { metric: "Service location management", before: "Hardcoded per-service configuration", after: "Dynamic via Eureka Discovery Server" },
    ],
    results: [
      { label: "Data manipulation efficiency", value: "~30%", description: "Reduction in Accounts Service processing overhead via the Java Stream API" },
      { label: "Service uptime", value: "~99.9%", description: "Achieved through centralized config, discovery, and resilient service communication" },
      { label: "Independently deployable services", value: "8+", description: "Authentication, Account, Wallet, Payment, Transfer, Notification, Reporting, Gateway" },
    ],
    lessonsLearned: [
      "Getting authentication, gateway routing, and service discovery right before building out feature services paid off every time a new service was added — the foundational patterns did most of the work.",
      "Stateless JWT authorization scales cleanly across services, but it shifts the hard problem from 'where is the session' to 'how do we handle token revocation and expiry' — worth designing for deliberately rather than as an afterthought.",
      "Centralizing configuration removes a whole category of environment-drift bugs, but it makes the Config Server's own availability a platform-wide concern that deserves the same operational care as any core service.",
      "Performance work doesn't always need a new data model — rewriting the Accounts Service's collection processing with the Java Stream API delivered a meaningful, measurable improvement without touching the underlying schema.",
    ],
  },
  {
