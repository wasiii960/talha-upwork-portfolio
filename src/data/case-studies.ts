...
export const caseStudies: CaseStudy[] = [
  {
    ...
    challenges: [
      ...
      "The edge device had to run CNN inference fast enough on Raspberry Pi-class hardware, with no GPU and no cloud round-trip, since a network dependency was not acceptable on a production line.",
  ...
  },
  {
    ...
    lessonsLearned: [
      ...
      "Designing for parameterization from day one, covering trolley dimensions, speed, and tape color, paid off directly when the plant wanted to onboard additional trolley sizes with zero new development.",
  ...
  },
  {
    ...
    engineeringDecisions: [
      ...
      },
      {
        ...
        detail:
          "Rather than introducing a dedicated message queue system, an existing PostgreSQL table was used as a FIFO queue for tracking incoming CSV files. This kept the solution cost-efficient, with no new infrastructure, while still guaranteeing idempotent, ordered processing.",
      },
      {
  ...
  },
  {
    ...
    architectureNotes: [
      "A generalistic cycle-cutting algorithm, developed with production domain experts, splits each motor's continuous time-series into individual operating cycles regardless of that motor's specific cycle shape. This was the key step that let one pipeline scale across all 500+ motors.",
